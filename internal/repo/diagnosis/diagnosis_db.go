...
// GetActiveByVisitID joins diagnosis rows with ICD-10 display text and the
// doctor master to produce the read-model used by GET /v1/visit/:visit_id/diagnosis.
// It uses the slave DB because reads never participate in write transactions.
func (c *Conn) GetActiveByVisitID(ctx context.Context, institutionID, visitID int64) ([]model.TrxDiagnosisWithDoctor, error) {
	const sql = `
		SELECT
			d.id, d.visit_id, d.institution_id, d.doctor_id, d.icd10_code,
			...
			ri.display  AS icd10_display,
			ri.category AS icd10_category
		FROM mdl_trx_diagnosis d
		LEFT JOIN mdl_mst_doctor md ON md.id = d.doctor_id
		LEFT JOIN ref_icd10     ri ON ri.code = d.icd10_code
